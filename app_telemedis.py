...
# ================= MENU 2: DASHBOARD REAL-TIME & AI =================
    ...
    if df_pasien.empty:
        ...
        if df_medis.empty:
            ...
            # --- FITUR CETAK EXCEL (VERSI DOWNLOAD INTERNET) ---
            st.markdown("### 📥 Laporan Rekam Medis")
            
            # 1. Merakit Excel di belakang layar
            ...
            # 2. Mengubah Excel menjadi format yang bisa di-download ke HP
            import io
            buffer = io.BytesIO()
            wb.save(buffer)
            buffer.seek(0)
            
            # 3. Menampilkan Tombol Download Resmi
            st.download_button(
                label="📥 Unduh Laporan Excel ke HP/Laptop",
                data=buffer,
                file_name=f"Laporan_Medis_{pilihan_pasien}.xlsx",
